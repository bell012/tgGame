<template>
  <popShell v-model="visible" :transition-type="isMobile ? 'bottom-sheet' : 'modal'">
    <section
      ...
      :class="
        isMobile
          ? 'w-full rounded-t-[18px] bg-bg-1 px-4 pb-[calc(env(safe-area-inset-bottom)+24px)] pt-4'
          : 'mx-auto w-[420px] max-w-[calc(100vw-40px)] rounded-[12px] bg-bg-2 px-4 pb-5 pt-3 shadow-[0_18px_54px_rgba(0,0,0,0.32)]'
      "
    >
...
const valueClass = computed(() => {
  return props.isMobile ? 'text-[14px] leading-[20px]' : 'text-[16px]'
})
