<template>
  <div class="min-w-0">
    <button
      type="button"
      class="flex h-[38px] w-full min-w-0 scroll-my-2 items-center gap-1.5 rounded-lg border border-solid bg-input-3 px-2 text-left focus-visible:outline focus-visible:outline-2 focus-visible:outline-theme-primary disabled:opacity-50"
      :class="
        props.error
          ? 'border-secondary-2'
          : props.active
    ...
    <p v-if="props.error" class="pt-1 text-[11px] leading-4 text-secondary-2" role="alert">
      {{ props.error }}
    </p>
  </div>
</template>
