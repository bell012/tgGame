<template>
  <section class="shrink-0" aria-label="Local bet simulation result">
    <div class="relative mx-[18px] mt-[18px] h-[177px] text-center">
      <button
        ...
        aria-label="Close local simulation result"
        @click="emit('dismiss')"
      >
      ...
      <p class="absolute inset-x-0 top-[132px] text-sm font-bold leading-[17px]" role="status">
        {{
          props.state === 'success' ? 'Your bet has been placed successfully!' : 'Your bet failed!'
        }}
      </p>
    ...
    <div class="mt-[18px] flex flex-col gap-3 px-3 pb-[18px]">
      <button type="button" :class="buttonClass" @click="emit('history')">Go to My Bets</button>
      <button type="button" :class="buttonClass" @click="emit('reuse')">Reuse Selections</button>
      <button type="button" :class="buttonClass" @click="emit('share')">Share</button>
    </div>
  </section>
</template>
