  ...
  >
    ...
    >
      ...
      <div class="mt-2 flex h-4 min-w-0 items-center gap-6 text-xs leading-4">
        <span class="truncate text-text-2">{{ match.kickoff }}</span>
        <span class="flex shrink-0 items-center gap-1" :aria-label="`Corners ${match.cornerScore}`">
          <CornerIcon class="h-4 w-4" aria-hidden="true" />
          {{ match.cornerScore }}
        </span>
        <span class="shrink-0" :aria-label="`Half-time ${match.halfTimeScore}`">
          HT {{ match.halfTimeScore }}
        </span>
      </div>
      <div class="mt-3 space-y-2">
        <div
          v-for="team in [match.home, match.away]"
          :key="team.name"
          class="flex h-6 min-w-0 items-center gap-3"
        >
          ...
          <span class="flex shrink-0 items-center gap-1 text-xs font-bold leading-4">
            <span
