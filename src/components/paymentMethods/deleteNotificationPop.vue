  ...
  <PopShell v-model="visible" @close="handleClose">
    ...
    >
      <div class="flex items-center justify-between gap-4">
        ...
        >
          {{ t('withdraw.delete_receive_account_title') }}
        </h2>
        <button
          type="button"
      ...
      >
        {{ t('withdraw.delete_receive_account_desc', { name: label }) }}
      </p>
      <div class="mt-[30px] flex flex-col gap-3 sm:mt-6 sm:flex-row sm:gap-6">
        ...
        <button
          type="button"
          class="order-1 sm:order-2 flex h-10 w-full items-center justify-center rounded-lg bg-theme-primary text-sm font-bold text-text-4 sm:h-12"
          @click="
            () => {
              if (isMobile) {
                handleDelete()
